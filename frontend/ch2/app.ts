...
// interface NewsFeedView extends View {};
// interface NewsDetailView extends View {};
...
abstract class View {
    ...
    constructor(containerId: string, template: string) {
        ...
        if (!containerElement) {
            throw '최상위 컨테이너가 없어 UI 를 진행하지 못합니다.';
        }
...
class NewsFeedView extends View {
  ...
  `;
    ...
    render(): void {
        ...
        this.setTemplateData('next_page', String(store.currentPage < Math.ceil(this.feeds.length / PAGE_COUNT) ? store.currentPage + 1 : store.currentPage));
        this.updateView();
    }
